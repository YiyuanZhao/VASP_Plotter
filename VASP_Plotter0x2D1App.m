classdef VASP_Plotter0x2D1App < handle
    %
    % Usage:
    %     app.VASP_Plotter0x2D1App
    ...
    properties (Constant)
        AppPathBase = {'/VASP_Plotter - 1'};
        AppClass = 'VASP_Plotter0x2D1App';
        Increment = 1;
        Decrement = 0;
        Version = '13a';
    end
    ...
    methods
        ...
        function obj = VASP_Plotter0x2D1App()
        ...
        function startApp(obj)
            ...
            mlock;
            VASP_Plotter0x2D1App.refcount(obj.Increment);
            ...
            existVal = exist(fullfile(pwd,'VaspPlotter')); %#ok<EXIST>
            ...
            if (doesShadowExist && ~pathOne.equals(pathTwo))
                % We are trying to execute the wrong VaspPlotter
                errordlg(message('MATLAB:apps:runapp:WrongEntryPoint', 'VaspPlotter').getString, ...
            ...
            if nargout(@VaspPlotter) == 0
                eval('VaspPlotter');
            else
                obj.AppHandle = eval('VaspPlotter');
            end
